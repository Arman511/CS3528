...
def test_register_employer(database, employer_model, app):
    ...
    with app.app_context():
        ...
        assert database.get_one_by_field("employers", "email", "contact@techcorp.com") is not None
...
def test_employer_login(database, employer_model, app, sample_employer):
    ...
    with app.app_context():
        with app.test_request_context():
            ...
            assert response[1] == 200
            assert response[0].json["message"] == "OTP sent if valid"
...
def test_rank_preferences(database, employer_model, app):
    ...
    with app.app_context():
        ...
        assert response[1] == 200
        updated_opportunity = database.get_one_by_id("opportunities", opportunity["_id"])
