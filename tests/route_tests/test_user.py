...
def test_update_employer_get_wrong_id(user_logged_in_client, database):
    ...
    response = user_logged_in_client.get(url, follow_redirects=True)  # Follow the redirect

    assert response.status_code == 200  # Since it redirects, it should be 200 after landing
